...
#   - host service ports from `.env` (backend, relay)
...
#
# Backend / relay run on the developer host (not in docker).
...
EOF
    success "生成 Traefik 配置 (backend=:${BACKEND_HTTP_PORT} relay=:${RELAY_HTTP_PORT})"
}
...
generate_env() {
    ...
    if [[ -f "$ENV_FILE" ]] && grep -q "COMPOSE_PROJECT_NAME=$project_name" "$ENV_FILE"; then
        ...
        if grep -q "^WEB_ADMIN_PORT=" "$ENV_FILE"; then
            sed -i.bak '/^WEB_ADMIN_PORT=/d' "$ENV_FILE"
            rm -f "$ENV_FILE.bak"
            unset WEB_ADMIN_PORT
            info "移除已退役的 WEB_ADMIN_PORT 配置"
        fi
