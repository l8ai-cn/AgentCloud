...
start_backend_host_lite() {
    ...
    export CORS_ALLOWED_ORIGINS="http://localhost:${HTTP_PORT},http://127.0.0.1:${HTTP_PORT},http://localhost:${WEB_PORT},http://127.0.0.1:${WEB_PORT},http://localhost:${MOBILE_LOVABLE_PORT:-10021},http://127.0.0.1:${MOBILE_LOVABLE_PORT:-10021},${PUBLIC_WEB_URL}"
    ...
    export COORDINATOR_RUNNER_DOCKER_COMPOSE_SERVICES="claude-code=runner-claude-code,codex-cli=runner-codex-cli,pattern-designer=runner-codex-cli,video-studio=runner-video-studio,cursor-cli=runner-cursor-cli,gemini-cli=runner-gemini-cli,kimi-code=runner-kimi-code,e2e-echo=runner-e2e-echo,loopal=runner-loopal,do-agent=runner-do-agent,seedance-expert=runner-do-agent,grok-build=runner-grok-build,minimax-cli=runner-minimax-cli,openclaw=runner-openclaw,hermes=runner-hermes,pi-agent=runner-pi-agent,aider=runner-aider,opencode=runner-opencode"
...
start_relay_host_lite() {
    ...
    export USE_HTTPS="${USE_HTTPS:-false}"
    export ALLOWED_ORIGINS="http://localhost:${HTTP_PORT},http://127.0.0.1:${HTTP_PORT},http://localhost:${WEB_PORT},http://127.0.0.1:${WEB_PORT},http://localhost:${MOBILE_LOVABLE_PORT:-10021},http://127.0.0.1:${MOBILE_LOVABLE_PORT:-10021},${PUBLIC_WEB_URL}"
