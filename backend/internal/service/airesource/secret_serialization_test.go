...
func TestCipherErrorsNeverExposeSensitiveMaterial(t *testing.T) {
	...
	service, err := NewService(Dependencies{Repository: f.repo, Cipher: failingCipher{encryptErr: errors.New(leakedPlaintext)}, Members: f.members, Prober: f.prober, Lister: f.lister, Mutations: f.mutations, Endpoints: allowingEndpoints{}, Grants: f.grants})
	require.NoError(t, err)
	...
	service, err = NewService(Dependencies{Repository: f.repo, Cipher: failingCipher{decryptErr: errors.New(leakedCiphertext)}, Members: f.members, Prober: f.prober, Lister: f.lister, Mutations: f.mutations, Endpoints: allowingEndpoints{}, Grants: f.grants})
	require.NoError(t, err)
