...
| Definition | `config/worker-types/do-agent/definition.json` |
| AgentFile | `config/worker-types/do-agent/AgentFile` |

## Runtime image

`do-agent-stable` enabled=true availability=locked_available
...
| Name | Flags | Value |
| --- | --- | --- |
| `DO_AGENT_HOME` | — | `sandbox.root + "/do-agent-home"` |
| `DO_AGENT_SETTINGS` | — | `sandbox.root + "/do-agent-home/settings.json"` |
| `DO_AGENT_LOG_DIR` | — | `sandbox.root + "/do-agent-home/logs"` |
...
| `MINIMAX_VIDEO_API_KEY` | SECRET OPTIONAL | `` |
| `MINIMAX_VIDEO_BASE_URL` | TEXT OPTIONAL | `` |
| `MINIMAX_VIDEO_MODEL` | TEXT OPTIONAL | `` |
...
- `minimax-video` modality=video capability=video-generation required=false
