...
import { buildSsoAuthUrl, getPreferredSsoDomain } from "@/lib/preferred-sso";
import { OAuthButtons } from "./OAuthButtons";
import { PreferredSsoEntry } from "./PreferredSsoEntry";
...
export function LocalLoginForm() {
  ...
  const preferredAuthUrl = preferredSsoDomain
    ? buildSsoAuthUrl(preferredSsoDomain, "oidc", searchParams.get("redirect"))
    : null;
  ...
  const discoverSSO = useCallback(async (usernameValue: string) => {
    ...
    try {
      ...
      // Silent — discovery failures must not block password login.
    }
  }, []);
